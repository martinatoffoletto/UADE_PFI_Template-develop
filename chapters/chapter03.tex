...
\nuevo{La Figura~\ref{fig:encuesta} contrasta los indicadores de los dos lados del mercado, que la plataforma necesita validados de forma simultánea para operar.}

\begin{figure}[H]
    \centering\color{nuevo}
    \begin{tikzpicture}[
        x=0.066cm, y=1cm,
        rot/.style={font=\scriptsize, text=black!85, anchor=east, align=right, text width=5.2cm},
        val/.style={font=\scriptsize\bfseries, anchor=west, inner sep=2.5pt},
        blq/.style={font=\footnotesize\bfseries, anchor=west, text=black!85},
        eje/.style={font=\scriptsize, text=black!50, anchor=north},
    ]
        % Guías verticales y escala
        \foreach \x in {25,50,75,100} {
            \draw[black!18, line width=0.4pt] (\x,-5.28) -- (\x,0.62);
            \node[eje] at (\x,-5.34) {\x};
        }
        \draw[black!40, line width=0.6pt] (0,-5.28) -- (0,0.62);
        \node[eje] at (0,-5.34) {0};
        \node[eje, anchor=north west] at (103,-5.34) {\%};

        % ── Bloque 1: demanda ──────────────────────────────────────────────
        \node[blq] at (0,0.36)
            {DEMANDA \textemdash{} remitentes \textnormal{\scriptsize(n = 116)}};
        \foreach [count=\i] \lbl/\v/\txt in {
            {Usaría la clasificación por foto}/80.2/{80,2},
            {Elige el servicio por costo}/75.0/{75,0},
            {Predispuesto al modelo colaborativo}/69.8/{69,8},
            {Valora conocer el CO\textsubscript{2} ahorrado}/53.4/{53,4}}
        {
            \pgfmathsetmacro{\y}{0.30-0.62*\i}
            \node[rot] at (-8,\y) {\lbl};
            \fill[black!78, rounded corners=1.5pt] (0,\y-0.17) rectangle (\v,\y+0.17);
            \node[val, text=black!78] at (\v,\y) {\txt\,\%};
        }

        % ── Bloque 2: oferta ───────────────────────────────────────────────
        \draw[black!22, line width=0.5pt] (-80,-2.55) -- (110,-2.55);
        \node[blq] at (0,-2.90)
            {OFERTA \textemdash{} transportistas potenciales};
        \foreach [count=\i] \lbl/\v/\txt in {
            {Motivado por un ingreso extra sin desvío \textnormal{\scriptsize(n = 98)}}/91.8/{91,8},
            {Interesado en transportar \textnormal{\scriptsize(n = 116)}}/84.5/{84,5}}
        {
            \pgfmathsetmacro{\y}{-2.96-0.62*\i}
            \node[rot] at (-8,\y) {\lbl};
            \fill[black!50, rounded corners=1.5pt] (0,\y-0.17) rectangle (\v,\y+0.17);
            \node[val, text=black!60] at (\v,\y) {\txt\,\%};
        }
        % Indicador negativo: relleno claro con contorno, para que no se lea como adhesión
        \pgfmathsetmacro{\y}{-2.96-0.62*3}
        \node[rot] at (-8,\y) {Preocupado por la responsabilidad\\ante daños \textnormal{\scriptsize(n = 94)}};
        \draw[fill=black!12, draw=black!55, line width=0.7pt, rounded corners=1.5pt]
            (0,\y-0.17) rectangle (73.4,\y+0.17);
        \node[val, text=black!60] at (73.4,\y) {73,4\,\%};
    \end{tikzpicture}
    \caption[Indicadores clave de la encuesta, por lado del mercado]{Indicadores clave de la encuesta, por lado del mercado. La barra de responsabilidad ante daños se dibuja contorneada porque mide una barrera de adopción y no una adhesión: es la proporción de transportistas potenciales que declara ese riesgo como su principal preocupación}
    \label{fig:encuesta}
\end{figure}
...
\begin{table}[htbp]
	...
	\begin{tabular}{p{3.5cm}p{5.1cm}p{5.4cm}}
		...
		Eleva la falta de seguimiento (50 menciones) de fricción frecuente a motivo de descarte. Confirma el subsistema de seguimiento (RF-TRK) para la primera versión utilizable \\
		\addlinespace
...
A partir de los datos obtenidos en la investigación de usuarios se construyeron tres \emph{user personas}, que sintetizan los perfiles más relevantes para DePaso y guían las decisiones de diseño de la plataforma:
...
\begin{minipage}[c]{0.72\textwidth}
    {\textbf{Juan García}} \\
    34 años \textperiodcentered{} Dueño de PyME \textperiodcentered{} Lanús, Buenos Aires
\end{minipage}
...
\begin{minipage}[c]{0.72\textwidth}
    {\textbf{María Álvarez}} \\
    40 años \textperiodcentered{} Trabaja en recursos humanos en el Microcentro \textperiodcentered{} Caballito, CABA
\end{minipage}
...
\begin{minipage}[c]{0.72\textwidth}
    {\textbf{Carlos Gómez}} \\
    55 años \textperiodcentered{} Fletero independiente \textperiodcentered{} Quilmes, Buenos Aires
\end{minipage}
...
\begin{itemize}[leftmargin=2em]
  ...
  \item\textbf{Cita representativa} \hfill \\
    \textit{«Quisiera una forma más estructurada de conseguir trabajo y, al mismo tiempo, un espacio con referencias que respalden mi servicio ante quien todavía no me conoce.»}
\end{itemize}
