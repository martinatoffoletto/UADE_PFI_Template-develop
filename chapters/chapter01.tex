...
{\color{nuevo}
\begin{itemize}[leftmargin=2em]
    \item Desarrollar e implementar un sistema de visión computacional capaz de clasificar automáticamente, a partir de una fotografía, las cargas en las cuatro categorías volumétricas definidas, alcanzando una exactitud (\emph{accuracy}) sobre el conjunto de prueba de al menos el 70~\%, con la exigencia adicional de que los errores se concentren en categorías adyacentes. El umbral responde a dos condiciones del problema: la estimación del volumen a partir de una única imagen es intrínsecamente ambigua, dado que una fotografía no conserva la escala absoluta de la escena \parencite{NaumannEtAl2023}; y la clasificación opera como apoyo a la decisión y no de forma autónoma, ya que el sistema expone la categoría inferida y admite su corrección manual, de modo que un error no deriva en una consecuencia irreversible.

    \item Implementar un mecanismo de asignación inteligente que evalúe la compatibilidad entre pedidos y transportistas considerando características de la carga, trayectorias, ventanas horarias y reputación, y que restrinja el desvío del transportista colaborativo a un máximo del 15~\% del trayecto original mediante un filtro duro, devolviendo un \emph{ranking} de candidatos con su puntaje desglosado y explicable.
...
El presente documento se organiza de la siguiente manera: el Capítulo~1 introduce el problema, las modalidades de servicio, los objetivos y el alcance del proyecto; el Capítulo~2 presenta los antecedentes, compuestos por el marco teórico, el estado del arte y {\color{nuevo}el análisis competitivo}; el Capítulo~3 describe la propuesta y la investigación de usuarios realizada; {\color{nuevo}el Capítulo~4 detalla el análisis de requerimientos, funcionales y no funcionales, junto con los casos de uso; el Capítulo~5 aborda la metodología de desarrollo, la arquitectura y las tecnologías, el modelo de datos y la validación del sistema; y el Capítulo~6} expone la conclusión parcial y el trabajo futuro. Completan el documento la bibliografía y los anexos, que incluyen el cronograma de actividades{\color{nuevo}, la transcripción de los resultados de la encuesta y la de las entrevistas semiestructuradas}.